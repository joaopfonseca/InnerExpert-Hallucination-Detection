...
def generate_confidence_distribution_tex(
    dfs: Dict[str, pd.DataFrame], label_value: int = 1
) -> str:
    """Confidence distribution table for labels equal to *label_value*."""
    label_name = "hallucinated" if label_value == 1 else "grounded"
    lines: List[str] = []
    lines.append(r"% Auto-generated by experiments/7.0-train-data-analysis.py")
    lines.append(r"% Requires: booktabs, xcolor")
    lines.append("")
    ...
    lines.append(r"\centering")
    lines.append(
        r"\caption{Confidence distribution for %s labels (label=%d)}"
        % (label_name, label_value)
    )
    lines.append(
        r"\label{tab:confidence-distribution-%s}" % label_name
    )
    ...
    bins = [(i * 0.2, (i + 1) * 0.2) for i in range(5)]
    bin_labels = [
    ...
    for (lo, hi), blabel in zip(bins, bin_labels):
        ...
        for model_key in ["olmoe", "gemma"]:
            df = dfs[model_key]
            sub = df[df["label_llm_answer"] == label_value]
            total = len(sub)
            n = int(
                (
                    (sub["label_hallucination_confidence"] >= lo)
                    & (sub["label_hallucination_confidence"] < hi)
                ).sum()
            )
            pct = 100.0 * n / total if total > 0 else 0.0
            counts[model_key] = (n, pct, total)
    ...
    for model_key in ["olmoe", "gemma"]:
        df = dfs[model_key]
        totals[model_key] = int((df["label_llm_answer"] == label_value).sum())
    lines.append(
...
def main() -> None:
    ...
    table_generators = [
        ("label_distribution.tex", lambda dfs: generate_label_distribution_tex(dfs)),
        ("confidence_distribution_hallucinated.tex", lambda dfs: generate_confidence_distribution_tex(dfs, label_value=1)),
        ("confidence_distribution_grounded.tex", lambda dfs: generate_confidence_distribution_tex(dfs, label_value=0)),
        ("label_asymmetry.tex", lambda dfs: generate_label_asymmetry_tex(dfs)),
    ]
    for name, gen_fn in table_generators:
        tex = gen_fn(dfs)
