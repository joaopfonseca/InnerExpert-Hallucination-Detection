...
# ---------------------------------------------------------------------------
def generate_confidence_table_tex(dfs: Dict[str, pd.DataFrame]) -> str:
    """Generate the confidence_table.tex fragment with label-distribution
    and confidence-distribution tables."""
    lines: List[str] = []
    lines.append(r"% Auto-generated by experiments/7.0-train-data-analysis.py")
    lines.append(r"% Requires: booktabs, xcolor")
    lines.append("")

    # --- Table 1: label distribution by condition ---
    ...
    for model_key in ["olmoe", "gemma"]:
        df = dfs[model_key]
        total = int((df["label_llm_answer"] == 1).sum())
        counts[model_key] = total
    lines.append(
        r"\textbf{Total} & \textbf{%d} & \textbf{100\,\%%} & \textbf{%d} & \textbf{100\,\%%} \\"
        % (counts["olmoe"], counts["gemma"])
    )
    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}")
    lines.append(r"\end{table}")
    ...
    # --- Table 3: label asymmetry summary ---
    metrics = []
    for model_key in ["olmoe", "gemma"]:
        df = dfs[model_key]
        total = len(df)
...
def main() -> None:
    ...
    print("Generating confidence_table.tex...")
    confidence_tex = generate_confidence_table_tex(dfs)
    (ANALYSIS_DIR / "confidence_table.tex").write_text(confidence_tex, encoding="utf-8")
    print(f"  -> {ANALYSIS_DIR / 'confidence_table.tex'}")
