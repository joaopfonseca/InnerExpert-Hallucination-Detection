...
- vs. **Token-Level Mahalanobis / TOHA**: although all three are single-pass internal-signal detectors, the latter two are **sequence-level** methods that require materializing very large volumes of internal state at inference time (per-layer hidden states and inverse covariance matrices for Mahalanobis; full per-layer, per-head attention matrices for TOHA) and collapse their signals into a single per-response score (mean-pool for Mahalanobis, persistent homology on the full response graph for TOHA). The storage footprint of running either of them is not feasible inside our lightweight test-time pipeline, and both are fundamentally incompatible with per-token hallucination localization. We cite both in the related work as the closest *sequence-level* cousins of our per-token MoE routing method, but exclude them from the headline comparison — see `docs/baselines.md` § *Cited but Not Evaluated Baselines* for the full rationale.
